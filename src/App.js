...
function App() {
  ...
  const contractAddress = '0x0CbB570f1141058C32F2D56c3201d568CFe7fD80';
  ...
  async function mintNFT(metadataURL) {
    ...
    const transaction = contract.methods.mint(metadataURL);
    const gas = await transaction.estimateGas({ from: account });
    const gasPrice = await web3.eth.getGasPrice();
    transaction.send({ from: account, gas, gasPrice });
  }
  ...
  async function handleFileUpload(event) {
    ...
    if (file) {
      ...
      console.log('IPFS URL:', result.ipfsHash);
